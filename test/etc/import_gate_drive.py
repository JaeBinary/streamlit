"""Gate Drive 테스트 시트(.xlsx) → cg_progress.db 1회성 이관 스크립트.

form/WG250-CG-Gate Drive Test sheet.xlsx 의 'Gate Driver' 시트(가로형: 열=Serial,
행=Test Item)를 읽어 기존 test_results 테이블에 적재한다. 기존 H-Bridge 데이터와의
키 충돌을 피하기 위해 serial은 'G'+4자리(G0021~)로 저장한다.

저장 포맷·SQL은 database.insert_records()와 동일하게 맞춘다(INSERT OR REPLACE).
G 접두어 + INSERT OR REPLACE라 재실행해도 중복 없이 갱신만 되어 멱등하다.
"""
import sqlite3
...
XLSX_PATH = BASE / "form" / "WG250-CG-Gate Drive Test sheet.xlsx"
DB_PATH = BASE / "data" / "cg_progress.db"
SHEET = "Gate Driver"

SAVED_BY = "jaebin.kim@gwanakanalog.com"  # 현재 사용자
SERIAL_PREFIX = "G"                        # Gate Driver 보드 구분 접두어

# 헤더 행 위치 (1-base)
ROW_SERIAL, ROW_DATE, ROW_TESTEDBY = 2, 3, 4
ROW_DATA_START = 6
COL_TEST_ITEM = 3                          # C열
TEST_ITEM_FIX = {"T": 26}                  # 시트 입력 오타 보정 (T → 26)
...
def extract_rows():
    ...
    serial_cols = {}  # col_idx -> serial 문자열(G0021)
    for c in range(1, ws.max_column + 1):
        v = ws.cell(row=ROW_SERIAL, column=c).value
        if isinstance(v, (int, float)):
            serial_cols[c] = f"{SERIAL_PREFIX}{int(v):04d}"

    # 저장 순서를 serial 우선으로 한다: G0021의 test_item 1~44 → G0022의 1~44 → …
...
def main():
    ...
    try:
        with conn:
            # serial 우선 순서로 다시 쌓기 위해 기존 G 행을 먼저 비운다(저장 순서 보장).
