"""Bypass Capacitor 테스트 시트(.xlsx) → cg_progress.db 1회성 이관 스크립트.

form/WG250-CG-Bypass and Tuning Capacitor Test sheet.xlsx 의 'Bypass Cap' 시트
(가로형: 열=Serial, 행=Test Item)를 읽어 기존 test_results 테이블에 적재한다.
H-Bridge('H')·Gate Driver('G') 데이터와의 키 충돌을 피하기 위해 serial은
'B'+4자리(B0021~)로 저장한다.

저장 포맷·SQL은 database.insert_records()·import_gate_drive.py와 동일하게 맞춘다
(INSERT OR REPLACE). B 접두어 + INSERT OR REPLACE라 재실행해도 중복 없이 갱신만
되어 멱등하다.
"""
import sqlite3
...
SERIAL_PREFIX = "B"                        # Bypass Capacitor 보드 구분 접두어
...
def _normalize_test_item(raw) -> str:
    """Test Item 셀값을 문자열로 정규화. 1.0 같은 실수는 정수로 떨군다."""
...
def extract_rows():
    ...
    # ('Serial No.' 같은 문자열 헤더 칸은 자동 제외된다.)
    serial_cols = {}  # col_idx -> serial 문자열(B0021)
    for c in range(1, ws.max_column + 1):
        v = ws.cell(row=ROW_SERIAL, column=c).value
        if isinstance(v, (int, float)):
            serial_cols[c] = f"{SERIAL_PREFIX}{int(v):04d}"

    # 저장 순서를 serial 우선으로 한다: B0021의 test_item 1~6 → B0022의 1~6 → …
    ...
    for c, serial in serial_cols.items():
        ...
        for r in range(ROW_DATA_START, ws.max_row + 1):
            ...
            if not test_item.isdigit():
                continue  # 'Overall Result (PASS/FAIL)' 같은 비-숫자 행은 제외

            # 측정값 없는 칸(예: Measured Capacitance 행)도 행은 만들고 값만 NULL로 둔다.
            # → 모든 serial이 test_item 1~6을 빠짐없이 갖게 된다.
            meas = ws.cell(row=r, column=c).value
            meas = None if (meas is None or (isinstance(meas, str) and meas.strip() == "")) else str(meas)

            rows.append((serial, test_item, meas, test_date, str(tested_by)))
    return rows
...
def main():
    ...
    try:
        with conn:
            # serial 우선 순서로 다시 쌓기 위해 기존 B 행을 먼저 비운다(저장 순서 보장).
